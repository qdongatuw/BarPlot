...
class FigureWindow(tk.Toplevel):
    ...
    def plot(self):
        ...
        bx = self.ax.boxplot(self.datas, showcaps=self.is_cap, showfliers=False, whis=[5, 95], widths=self.option['bar_width'])

        for index, data in enumerate(self.datas):

            plt.setp(bx['boxes'][index], color=self.option['colors'][index])
            if self.is_cap:
                plt.setp(bx['caps'][index*2], color=self.option['colors'][index])
                plt.setp(bx['caps'][index*2+1], color=self.option['colors'][index])
            plt.setp(bx['whiskers'][index*2], color=self.option['colors'][index])
            plt.setp(bx['whiskers'][index * 2 + 1], color=self.option['colors'][index])

            plt.setp(bx['medians'][index], color=self.option['colors'][index])
            if self.is_raw.get():
