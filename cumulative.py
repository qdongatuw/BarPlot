...
from cumulative_ax import CumAx
...
class FigureWindow(tk.Toplevel):
    ...
    def save(self):
        path = self.option['path'] + self.option['sheet'] + 'Cumulative_plot' + '.' + self.option['save_format']
    ...
    def plot(self):
        ...
        for data in self.datas:
            CumAx(self.ax, data=data, color=self.option['colors'][index],
                  legend=self.groups[index])
            index += 1
